...
internal class PdfDrawer<T>
{
   ...
   private readonly int _documentsInRow;
   ...
   internal PdfDrawer(DataTable<T> dataTable,
      string fontName,
      int fontSize,
      ...
      PageSize pageSize)
   {
      ...
      {
         var records = dataTable.GetRecordsForExport()
                                .ToArray();
         double recordsMaxLength = 0;
         if (records.Length != 0)
         {
            recordsMaxLength = records.Select(x => graphics.MeasureString(x[t], Font(_fontSize))
                                                           .Width)
                                      .Max();
         }
         ...
         _columnWidths.Add(
            Math.Max(recordsMaxLength,
               graphics.MeasureString(t, Font(_fontSize, true))
                       .Width) + 5);
      }

      _documentsInRow = 1;

      rowHeight = graphics.MeasureString("Test", Font(_fontSize))
                          .Height + 4;
   ...
   internal void AddRow(IReadOnlyList<string> values,
      XFont font,
      bool gridUp,
      bool gridDown,
      bool gridLeft,
      bool gridRight)
   {
      ...
      var graphics = _graphicsList.TakeLast(_documentsInRow)
                                  .ToList();
      var currentGraphicsIndex = 0;
      var currentGraphics = graphics[currentGraphicsIndex];
      var cellHeight = currentGraphics.MeasureString("Test", font)
                                      .Height + 4;
      ...
      for (var index = 0; index < values.Count; index++)
      {
         ...
         currentGraphics.DrawString(value,
            font,
            XBrushes.Black,
            NormalizeX(_currentX + CELL_PADDING),
         ...
         if (gridUp)
         {
            currentGraphics.DrawLine(
               XPens.Black,
               NormalizeX(_currentX),
               NormalizeY(_currentY),
   ...
   internal void AddRow(string value, XFont font)
   {
      ...
      var graphics = _graphicsList.TakeLast(_documentsInRow)
                                  .First();
      var cellHeight = (int)graphics.MeasureString("Test", font)
                                    .Height + 4;
   ...
   {
      return new XFont(_fontName,
         fontSize,
         bold ? XFontStyle.Bold : XFontStyle.Regular,
